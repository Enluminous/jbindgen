...
public class CXEvalResult implements PtrOp<CXEvalResult, Void>, Info<CXEvalResult> {
    public static final Operations<Void> ELEMENT_OPERATIONS = Info.makeOperations();
    public static final Operations<CXEvalResult> OPERATIONS = PtrOp.makeOperations(CXEvalResult::new);
    ...
    public CXEvalResult(MemorySegment segment) {
        this.segment = fitByteSize(segment);
    }

    public CXEvalResult(ArrayOp<?, Void> arrayOperation) {
        this.segment = fitByteSize(arrayOperation.operator().value());
    }

    public CXEvalResult(Value<MemorySegment> pointee) {
        this.segment = fitByteSize(pointee.operator().value());
    }

    public CXEvalResult(PtrI<Void> pointee) {
        this.segment = fitByteSize(pointee.operator().value());
    }

    public static Array<CXEvalResult> list(SegmentAllocator allocator, long len) {
        return new Array<>(allocator, CXEvalResult.OPERATIONS, len);
    }
    ...
    @Override
    public PtrOpI<CXEvalResult, Void> operator() {
        return new PtrOpI<>() {
            @Override
            ...
            @Override
            public Operations<CXEvalResult> getOperations() {
                return OPERATIONS;
            }
