    ...
    void EnumGenerator::build() {
        ...
        for (auto &enumDeclaration: enumDeclarations) {
            ...
            body += std::vformat("    public static final class {0} extends VI32Basic<{0}> {{\n"
                                 ...
                                 "        }}\n"
                                 "\n"
                                 "        public static <T> VI32List<{0}> list(Pointer<{0}> ptr) {{\n"
                                 "            return new VI32List<>(ptr, {0}::new);\n"
                                 "        }}\n"
                                 "\n"
                                 "        public static <T> VI32List<{0}> list(Pointer<{0}> ptr, long length) {{\n"
                                 "            return new VI32List<>(ptr, length, {0}::new);\n"
                                 "        }}\n"
                                 "\n"
                                 "        public static <T> VI32List<{0}> list(Pointer<{0}> ptr, long length, Arena arena, Consumer<Pointer<{0}>> cleanup) {{\n"
                                 "            return new VI32List<>(ptr, length, arena, cleanup, {0}::new);\n"
                                 "        }}\n"
                                 "\n"
                                 "        public static <T> VI32List<{0}> list(Arena arena, long length) {{\n"
                                 "            return new VI32List<>(arena, length, {0}::new);\n"
                                 "        }}\n"
                                 "\n"
                                 "        public static <T> VI32List<{0}> list(Arena arena, {0}[] c) {{\n"
                                 "            return new VI32List<>(arena, c, {0}::new);\n"
                                 "        }}\n"
                                 "\n"
                                 "        public static <T> VI32List<{0}> list(Arena arena, Collection<{0}> c) {{\n"
                                 "            return new VI32List<>(arena, c, {0}::new);\n"
                                 "        }}\n"
                                 "\n"
                                 "        private String str;\n"
