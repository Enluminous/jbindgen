...
namespace jbindgen {
    ...
    std::string getAbstractNativeListContent() {
        return "\n"
               ...
               "    }\n"
               "\n"
               "    public AbstractNativeList(Pointer<T> ptr, long length, Arena arena, Consumer<Pointer<T>> cleanup, Function<Pointer<T>, T> constructor, long elementByteSize) {\n"
               "        this(ptr.pointer().reinterpret(length * elementByteSize, arena, memorySegment -> cleanup.accept(() -> memorySegment)), constructor, elementByteSize);\n"
               "    }\n"
               "\n"
               "    public AbstractNativeList(Pointer<T> ptr, Arena arena, Consumer<Pointer<T>> cleanup, Function<Pointer<T>, T> constructor, long elementByteSize) {\n"
               "        this(ptr.pointer().reinterpret(arena, memorySegment -> cleanup.accept(() -> memorySegment)), constructor, elementByteSize);\n"
               "    }\n"
               "\n"
    ...
    std::string getNListContent() {
        return "\n"
               ...
               "    }\n"
               "\n"
               "    public NList(Pointer<E> ptr, long length, Arena arena, Consumer<Pointer<E>> cleanup, Function<Pointer<E>, E> constructor, long elementByteSize) {\n"
               "        super(ptr, length, arena, cleanup, constructor, elementByteSize);\n"
               "    }\n"
               "\n"
               "    public NList(Pointer<E> ptr, Arena arena, Consumer<Pointer<E>> cleanup, Function<Pointer<E>, E> constructor, long elementByteSize) {\n"
    ...
    std::string getNPtrListContent() {
        return "\n"
               ...
               "    }\n"
               "    \n"
               "    public NPtrList(Pointer<T> ptr, Function<Pointer<T>, T> constructor, long elementByteSize, long length) {\n"
               "        this(ptr.pointer().reinterpret(length * ValueLayout.ADDRESS.byteSize()), constructor, elementByteSize);\n"
               "    }\n"
               "\n"
               "    public NPtrList(Pointer<T> ptr, Function<Pointer<T>, T> constructor, long[] elementsByteSize) {\n"
               "        this(ptr.pointer().reinterpret(elementsByteSize.length * ValueLayout.ADDRESS.byteSize()), constructor, elementsByteSize);\n"
               "    }\n"
               "\n"
               "\n"
               "    public NPtrList(Pointer<T> ptr, Arena arena, Consumer<Pointer<T>> cleanup, Function<Pointer<T>, T> constructor, long elementByteSize, long length) {\n"
               "        this(ptr.pointer().reinterpret(length * ValueLayout.ADDRESS.byteSize(), arena, memorySegment -> cleanup.accept(() -> memorySegment)),\n"
               "                constructor, elementByteSize);\n"
               "    }\n"
               "\n"
               "    public NPtrList(Pointer<T> ptr, Arena arena, Consumer<Pointer<T>> cleanup, Function<Pointer<T>, T> constructor, long[] elementsByteSize) {\n"
               "        this(ptr.pointer().reinterpret(elementsByteSize.length * ValueLayout.ADDRESS.byteSize(), arena,\n"
               "                memorySegment -> cleanup.accept(() -> memorySegment)), constructor, elementsByteSize);\n"
               "    }\n"
               "\n"
               "    public NPtrList(Pointer<T> ptr, Arena arena, Consumer<Pointer<T>> cleanup, Function<Pointer<T>, T> constructor, long elementByteSize) {\n"
               "        this(ptr.pointer().reinterpret(arena, memorySegment -> cleanup.accept(() -> memorySegment)), constructor, elementByteSize);\n"
               "    }\n"
               "\n"
               ...
               "    }\n"
               "\n"
               "    public NPtrList(Pointer<T> ptr, Function<Pointer<T>, T> constructor, long elementByteSize) {\n"
               "        super(ptr, constructor, ValueLayout.ADDRESS.byteSize());\n"
    ...
    [[deprecated("")]]std::string getVListContent() {
        return "\n"
               "import java.lang.foreign.Arena;\n"
               "import java.lang.foreign.MemorySegment;\n"
               "import java.lang.foreign.ValueLayout;\n"
    ...
    std::string getSubValueContent(std::string className, std::string basicClassName, std::string specializedList,
        ...
        return std::vformat(
                ...
                "    }}\n"
                "\n"
                "    public static <T> {5}<{3}<T>> list(Pointer<{3}<T>> ptr, long length, Arena arena, Consumer<Pointer<{3}<T>>> cleanup) {{\n"
                "        return new {5}<>(ptr, length, arena, cleanup, {3}::new);\n"
                "    }}\n"
                "\n"
                "    public static <T> {5}<{3}<T>> list(Arena arena, long length) {{\n"
                "        return new {5}<>(arena, length, {3}::new);\n"
                "    }}\n"
                "\n"
                "    public static <T> {5}<{3}<T>> list(Arena arena, {3}<T>[] c) {{\n"
                "        return new {5}<>(arena, c, {3}::new);\n"
                "    }}\n"
                "\n"
                "    public static <T> {5}<{3}<T>> list(Arena arena, Collection<{3}<T>> c) {{\n"
                "        return new {5}<>(arena, c, {3}::new);\n"
                "    }}\n"
                "\n"
    ...
    getSubValueContentSpecialized(std::string className, std::string basicClassName, std::string specializedList,
        ...
        return std::vformat(
                ...
                "    }}\n"
                "\n"
                "    public static {5}<{3}> list(Pointer<{3}> ptr, long length, Arena arena, Consumer<Pointer<{3}>> cleanup) {{\n"
                "        return new {5}<>(ptr, length, arena, cleanup, {3}::new);\n"
                "    }}\n"
                "\n"
                "    public static {5}<{3}> list(Arena arena, long length) {{\n"
                "        return new {5}<>(arena, length, {3}::new);\n"
                "    }}\n"
                "\n"
                "    public static {5}<{3}> list(Arena arena, {3}[] c) {{\n"
                "        return new {5}<>(arena, c, {3}::new);\n"
                "    }}\n"
                "\n"
                "    public static {5}<{3}> list(Arena arena, Collection<{3}> c) {{\n"
                "        return new {5}<>(arena, c, {3}::new);\n"
                "    }}\n"
                "\n"
    ...
    std::string getVListSpecializedContent(std::string className, std::string valueLayout, std::string byteSize,
        ...
        return std::vformat(
                ...
                "    }}\n"
                "\n"
                "    public {0}(Pointer<T> ptr, long length, Arena arena, Consumer<Pointer<T>> cleanup, Function<Pointer<T>, T> constructor) {{\n"
                "        super(ptr, length, arena, cleanup, constructor, ELEMENT_BYTE_SIZE);\n"
                "    }}\n"
                "\n"
                "    public {0}(Arena arena, long length, Function<Pointer<T>, T> constructor, BiConsumer<Long, {0}<T>> creator) {{\n"
                "        super(arena.allocate(ELEMENT_BYTE_SIZE * length, 4), constructor, ELEMENT_BYTE_SIZE);\n"
                "        for (int i = 0; i < length; i++) {{\n"
                "            creator.accept((long) i, subList(i, i));\n"
                "        }}\n"
                "    }}\n"
                ...
                "        super(arena.allocate(length * ELEMENT_BYTE_SIZE), constructor, ELEMENT_BYTE_SIZE);\n"
                "    }}\n"
                "\n"
                "    public {0}(Arena arena, T[] objs, Function<Pointer<T>, T> constructor) {{\n"
                "        super(arena.allocate(objs.length * ELEMENT_BYTE_SIZE), constructor, ELEMENT_BYTE_SIZE);\n"
                "        long index = 0;\n"
                "        for (T obj : objs) {{\n"
                ...
                "    }}\n"
                "\n"
                "    public {0}(Arena arena, Collection<T> objs, Function<Pointer<T>, T> constructor) {{\n"
                "        super(arena.allocate(objs.size() * ELEMENT_BYTE_SIZE), constructor, ELEMENT_BYTE_SIZE);\n"
                "        long index = 0;\n"
                "        for (T obj : objs) {{\n"
                "            ptr.setAtIndex({2}, index, obj.value());\n"
                "            index++;\n"
    ...
    std::string getNStringContent() {
        return "\n"
               ...
               "        }\n"
               "\n"
               "        public NativeStringList(MemorySegment ptr, long length) {\n"
               "            this(ptr.reinterpret(length * ELEMENT_BYTE_SIZE));\n"
               "        }\n"
               "\n"
               "        public NativeStringList(MemorySegment ptr, long length, Arena arena, Consumer<MemorySegment> cleanUp) {\n"
               "            this(ptr.reinterpret(length * ELEMENT_BYTE_SIZE, arena, cleanUp));\n"
               "        }\n"
               "\n"
               "        public NativeStringList(Arena arena, String[] strings) {\n"
               "            this(arena.allocate(ValueLayout.ADDRESS, strings.length));\n"
               "            int i = 0;\n"
               "            for (String string : strings) {\n"
               "                ptr.setAtIndex(ValueLayout.ADDRESS, i, arena.allocateFrom(string));\n"
               "                i++;\n"
               "            }\n"
               "        }\n"
               "\n"
               "        public NativeStringList(Arena arena, Collection<String> strings) {\n"
               "            this(arena.allocate(ValueLayout.ADDRESS, strings.size()));\n"
               "            int i = 0;\n"
               "            for (String string : strings) {\n"
               "                ptr.setAtIndex(ValueLayout.ADDRESS, i, arena.allocateFrom(string));\n"
               "                i++;\n"
               ...
               "    }\n"
               "\n"
               "    public static NList<NString> list(MemorySegment ptr, long length) {\n"
               "        return new NativeStringList(ptr, length);\n"
               "    }\n"
               "\n"
               "    public static NList<NString> list(MemorySegment ptr, long length, Arena arena, Consumer<MemorySegment> cleanup) {\n"
               "        return new NativeStringList(ptr, length, arena, cleanup);\n"
               "    }\n"
               "\n"
               "    public static NList<NString> list(Arena arena, String[] strings) {\n"
               "        MemorySegment strPtr = arena.allocate(ValueLayout.ADDRESS, strings.length);\n"
               "        int i = 0;\n"
               "        for (String string : strings) {\n"
               "            strPtr.setAtIndex(ValueLayout.ADDRESS, i, arena.allocateFrom(string));\n"
               "            i++;\n"
               "        }\n"
               "        return new NativeStringList(strPtr);\n"
               "    }\n"
               "\n"
               "    public static NList<NString> list(Arena arena, Collection<String> strings) {\n"
               "        MemorySegment strPtr = arena.allocate(ValueLayout.ADDRESS, strings.size());\n"
               "        int i = 0;\n"
               "        for (String string : strings) {\n"
               "            strPtr.setAtIndex(ValueLayout.ADDRESS, i, arena.allocateFrom(string));\n"
               "            i++;\n"
               ...
               "    }\n"
               "\n"
               "    public NString(Arena arena, String s) {\n"
