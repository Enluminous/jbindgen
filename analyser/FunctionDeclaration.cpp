...
namespace jbindgen {
    FunctionDeclaration FunctionDeclaration::visit(CXCursor c) {
        ...
        Typed retType(NO_NAME, type, size, NO_COMMIT, c);
        Typed functionType(funcName, type, clang_Type_getSizeOf(type), getCommit(c), c);
        ...
        for (int i = 0; i < clang_getNumArgTypes(type); ++i) {
            ...
            Typed par(name, argType, clang_Type_getSizeOf(argType), NO_COMMIT, c);
            def.addPara(par);
        }
        return def;
    }
